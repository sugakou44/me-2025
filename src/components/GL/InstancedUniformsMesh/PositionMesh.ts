...
import type { Mesh as MeshProps } from './types'
...
export class PositionMesh extends Group {
  color: Color
  instancedMesh: CurrentWritable<MeshProps | undefined>
  instances: CurrentWritable<PositionMesh[]>

  constructor(
    instancedMesh: CurrentWritable<MeshProps>,
    instances: CurrentWritable<PositionMesh[]>,
  ) {
    super()
  ...
  override raycast(raycaster: Raycaster, intersects: Intersection[]): void {
    ...
    if (isInstanceOf(parent.material, 'Material')) {
      ...
      _mesh.material.side = (parent.material as Dict)[0]?.side ?? FrontSide
    }
