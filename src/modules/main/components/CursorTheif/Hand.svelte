<script lang="ts">
  ...
  const BOUNDS = 250 / 2
  const OVER_REACH_BOUNDS = 160 / 2
  const REACH_BOUNDS = 110 / 2
...
<div
  bind:this={container}
  class={cn(
    'absolute hidden aspect-square h-8 w-8 rounded-full md:block',
    className,
  )}
>
  <div
  ...
  >
    ...
    >
      ...
      <path
        ...
        class="fill-white drop-shadow-[0px_0px_400px_var(--color-tertiary)]"
      />
    </svg>
    {#if gotCursor}
      <IconPointerFilled
