<script lang="ts">
  ...
  let { boundSize = 150, class: className }: Props = $props()

  let container = $state<HTMLDivElement>()
  const DEG60 = utils.degToRad(60)
  const DIALOG_BOUND = 50
...
<svelte:window
  {@attach gestures({
    onMove: ({ xy: [mouseX, mouseY] }) => {
      ...
      eyeSpring.set({
        x: Math.round(diffX / 18),
        y: Math.round(diffY / 18),
      })
    },
  })}
/>
...
<div class="relative h-full w-full" bind:this={container}>
  <div class={cn('relative h-fit w-28', className)}>
    <div
      style:transform={`translate3d(calc(-50% + ${bodySpring.current.x}px), calc(-50% + ${bodySpring.current.y}px), 50px)`}
      class="absolute top-1/2 left-1/2 aspect-square h-28 w-28 perspective-dramatic"
    >
      <div
        style:transform={`translate3d(${eyeSpring.current.x}px, ${eyeSpring.current.y}px, 0px)`}
        class={cn(
          'absolute top-[30%] left-[20%] h-3 w-3 scale-y-140 rounded-full bg-tertiary-foreground  ',
          {
      ...
      <div
        style:transform={`translate3d(${eyeSpring.current.x}px, ${eyeSpring.current.y}px, 0px)`}
        class={cn(
          'absolute top-[30%] right-[20%] h-3 w-3 scale-y-140 rounded-full bg-tertiary-foreground  ',
          {
            'top-[35%] !h-1': gotCursor,
          },
        )}
      ></div>
      <div
        class="absolute right-[40%] bottom-[15%] rotate-90 font-mono text-[1.5rem] font-bold text-tertiary-foreground"
      >
        {#if gotCursor}
          <div>
            )
            ...
            >
              <IconBookmarkFilled class=" h-3 w-3" />
            </div>
          </div>
        {:else}
          <div>)</div>
      ...
      />
      <DialogBox
        position={[DIALOG_BOUND, 0]}
        heightOffset={60}
        label={gotCursor ? 'Gotcha' : ''}
      />
    </div>
  </div>
</div>
