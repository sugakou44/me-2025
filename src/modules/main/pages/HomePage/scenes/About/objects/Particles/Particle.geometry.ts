...
export class ParticleGeometry extends BufferGeometry {
  count = 0
  constructor(count: number, size: number, textureCount = 0) {
    super()
    ...
    for (let y = 0; y < size; y++) {
      for (let x = 0; x < size; x++) {
        ...
        const color = _COLORS[utils.random(0, 1)]
        colors[i3 + 0] = color.r
        colors[i3 + 1] = color.g
        colors[i3 + 2] = color.b

        textureIndices[i] = utils.random(0, textureCount - 1)
    ...
    this.setAttribute('textureIndex', new BufferAttribute(textureIndices, 1))
