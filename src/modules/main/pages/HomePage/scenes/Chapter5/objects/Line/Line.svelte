<script lang="ts">
  ...
  const points: number[] = []
  const pointsData = cameraTrack.cameraCurve.getSpacedPoints(TEXTURE_SIZE - 1)
  const frenetFramesData = cameraTrack.cameraCurve.computeFrenetFrames(
    TEXTURE_SIZE - 1,
  )
...
<T.Group rotation={[utils.degToRad(-90), 0, 0]} position={[0, 0, -10]}>
  <T.Mesh {geometry} frustumCulled={false}>
    <T is={material} uniforms.stretchRatio.value={windowState.scrollPercent} />
  </T.Mesh>
</T.Group>
